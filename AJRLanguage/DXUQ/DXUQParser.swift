...
public enum DXUQParseError : Error {
    case invalidInput(String)
}

public func Parse(_ string: String) throws -> ExprC {
    return try DXUQParser.parse(string)
}

public func Parse(_ expression: SExpression) throws -> ExprC {
    return try DXUQParser.parse(expression)
}

public func Parse(_ expressions: [SExpression]) throws -> [ExprC] {
    return try expressions.map { try DXUQParser.parse($0) }
}

open class DXUQParser: NSObject {
    ...
    open func parse(_ expression: SExpression) throws -> ExprC {
        ...
        if let expressions = expression as? ArrayExpression,
           ...
           let arguments = ArrayExpression.values(expressions[1], as: SymbolExpression.self) {
            ...
            if argumentNames.count != Set(argumentNames).count {
                throw DXUQParseError.invalidInput("Duplicate parameter names: \(argumentNames.componentsJoinedByString(separator: " ")).")
            }
        ...
        if let expressions = expression as? ArrayExpression,
           ...
           expressions.count == inIndex + 2 {
            ...
            //print(body)
            return AppC(function: LambdaC(args: declarationExpressions.map { return $0.symbol },
                                          body: body),
                        arguments: declarationExpressions.map { return $0.body})    
        }
        if let expressions = expression as? ArrayExpression,
           expressions.count > 0 {
        ...
        throw DXUQParseError.invalidInput("Unknown token in input.")
    }
    ...
    internal func buildLetDeclarations(declarations : ArraySlice<SExpression>) throws -> [LetTuple] {
        return try declarations.map { (expression) -> LetTuple in
            ...
            throw DXUQParseError.invalidInput("Bad let syntax")
        }
    }
    ...
    open class func parse(_ string: String) throws -> ExprC {
        ...
        if let expression = try DXUQLexer().lexSExpression(string) {
            return try DXUQParser().parse(expression)
        }
        return ValueC(VoidV())
    }
