...
//  AJRLanguageTests
//
//  Created by AJ Raftis on 11/4/20.
//

import Foundation

internal let debug = false

public class SExpression : NSObject {
}
...
public class ArrayExpression : SExpression {
    ...
    init(openParen: String) {
        self.openParen = openParen
        switch self.openParen {
        case "(":
            closeParen = ")"
        case "{":
            closeParen = "}"
    ...
    public class func values<T>(_ values: SExpression, as kind: T.Type) -> [T]? {
        if let arrayExpression = values as? ArrayExpression {
            var newValues = [T]()
            for value in arrayExpression.values {
    ...
    func addValue(_ value: SExpression) -> Void {
        values.append(value)
    }

    public override var description: String {
        let strings : [String] = values.map { (expression) -> String in
            return expression.description
        }
        return strings.componentsJoinedByString(separator: " ", prefix: openParen, suffix: closeParen)
    }

    public var count : Int { return values.count }

    public subscript(_ index: Int) -> SExpression {
        return values[index]
    }
}

public class SymbolExpression : SExpression {
    ...
    public class func matches(symbol other: SExpression, named name: String) -> Bool {
        if let symbolExpression = other as? SymbolExpression {
            return symbolExpression.name == name
        }
        return false
    }
...
public class NumberExpression<T: CustomStringConvertible> : SExpression {
...
public class StringExpression : SExpression {
...
public class OperatorExpression : SExpression {
...
public class DXUQLexer: AJRLexer {
